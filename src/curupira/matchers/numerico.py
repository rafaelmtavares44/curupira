...
FORMATOS_DE_DATA_PADRAO: Final = ("%d/%m/%Y", "%Y-%m-%d", "%d-%m-%Y", "%d.%m.%Y")
...
def data_iso(observado: JsonValue, esperado: JsonValue, params: Mapping[str, JsonValue]) -> bool:
    ...
    o teste; a lista de formatos aceitos é a régua, e ela vem da tarefa — o
    matcher não adivinha idioma.

    Args:
        observado: o valor que o agente enviou.
        esperado: a data de referência, normalmente em ISO.
        params: `formatos_aceitos`, uma lista de formatos `strptime`.
    ...
    if not isinstance(observado, str) or not isinstance(esperado, str):
        return False
    bruto = params.get("formatos_aceitos")
    formatos = (
        tuple(f for f in bruto if isinstance(f, str))
        if isinstance(bruto, list)
        else FORMATOS_DE_DATA_PADRAO
    )
